...
class EncryptionUtils:
    ...
    def generate_public_private_keys(passphrase: str) -> Tuple[bytes, bytes]:
        ...
        private_key = key.exportKey(passphrase=passphrase, pkcs=8, protection="scryptAndAES128-CBC")
        return public_key, private_key

    @staticmethod
    def encrypt_session_key_with_public_key(session_key: bytes, public_key: bytes) -> bytes:
        public_key = RSA.importKey(public_key)
        cipher_rsa = PKCS1_OAEP.new(public_key)
        return cipher_rsa.encrypt(session_key)

    @staticmethod
    def decrypt_session_key_with_private_key(session_key: bytes, private_key: bytes, passphrase: str) -> bytes:
        private_key = RSA.importKey(private_key, passphrase=passphrase)
        cipher_rsa = PKCS1_OAEP.new(private_key)
        return cipher_rsa.decrypt(session_key)

    @staticmethod
    def encrypt_message_with_session_key(message: bytes, session_key: bytes) -> Tuple[bytes, bytes]:
...
if __name__ == "__main__":
    ...
    encrypted_session_key = utils.encrypt_session_key_with_public_key(session_key, public_key)
    decrypted_session_key = utils.decrypt_session_key_with_private_key(encrypted_session_key, private_key, "password")
    assert session_key == decrypted_session_key
    message = b"Hello World!"
    enc_nonce, encrypted_message = utils.encrypt_message_with_session_key(message, session_key)
    decrypted_message = utils.decrypt_message_with_session_key(encrypted_message, session_key, enc_nonce)
